// FUNÇÃO 5 - VERIFICA SE O JOGADOR GANHOU 
#include <stdio.h>
#include <assert.h>
#include <time.h>
#include <stdbool.h>
...
int main() 
{
  srand(time(0));
  jogo_t jogo;
  jogo.pilha_saida[0].n_cartas = 13;
  jogo.pilha_saida[1].n_cartas = 13;
  jogo.pilha_saida[2].n_cartas = 13;
  jogo.pilha_saida[3].n_cartas = 13;
 
  if (jogador_ganhou(&jogo)) {
    printf("funcionou\n"); 
  } else printf("não funcionou");

  if (total_cartas_pilha(&jogo.pilha_saida[1]) == 13) {
    printf("o problema não tá na função total cartas");
  } else printf("o problema tá na função total cartas");
}
