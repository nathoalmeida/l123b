// FUNÇÃO 3 - RETORNA SE UMA CARTA PODE SER MOVIDA PARA A PILHA DE JOGO
...
// retorna se uma carta pode ser movida para a pilha de jogo
...
int main() 
{
  srand(time(0));
  jogo_t jogo;
  jogo.pilha_principal[0].n_cartas = 0;

  
  carta_t carta_teste1 = { rei, ouro};
  carta_t carta_teste2 = { dama, paus};
  carta_t carta_teste3 = { 4, paus};


  if (pode_mover_p_principal(&jogo, 0, carta_teste1)) {
    printf("o primeiro teste passou\n");
    empilha_carta(carta_teste1, &jogo.pilha_principal[0]);
  } else { printf("começou mal");}

  if (pode_mover_p_principal(&jogo, 0, carta_teste2)) {
    printf("o segundo teste passou\n");
  } else { printf("deu ruim de novo pra variar\n");}

  if (pode_mover_p_principal(&jogo, 0, carta_teste3)) {
    printf("o terceiro teste não passou\n");
  } else { printf("aleluia uma vez deu certo\n");}
